...
| `services/sherpa_service.dart`, `whisper_service.dart` | LO-32 wrapped them as `transcription/sherpa_streaming.dart` / `whisper_batch.dart`. LO-41 finished the sherpa side: the decode loop now runs in `transcription/sherpa_worker.dart` on a worker isolate, segments carry wall-clock `startAt/endAt` and token-relative `startTime/endTime`, and `sherpa_service.dart` has no *streaming* caller left — LO-40 took its model download away and LO-41 its decode loop, leaving only the file-transcription path (`SessionController._transcribeWithSherpa`), so it goes away once M5 puts `FileTranscriber` in front of that. LO-42 did the same for the whisper side: `whisper_batch.dart` now drives `transcription/whisper_worker.dart` on a worker isolate, cutting utterances with the Silero VAD in `transcription/vad.dart` instead of the old fixed 3-second timer, and stamps segments from the VAD's sample indices. That leaves `whisper_service.dart` with the same single caller `sherpa_service.dart` has — the file-transcription path (`SessionController._transcribeWithWhisper`) — so both go away together once M5 puts `FileTranscriber` in front of it. |
